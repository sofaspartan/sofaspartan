      ...
      >
        <div className="flex flex-col md:flex-row gap-4 md:gap-8 p-3 md:p-6">
          ...
          <div className="flex-shrink-0 mx-auto md:mx-0">
            <div 
              className="w-[200px] h-[200px] md:w-[320px] md:h-[320px] rounded-lg overflow-hidden shadow-lg transition-transform duration-100"
              style={{
            ...
            <div className="mt-3 md:mt-4 text-center">
              {currentTrack ? (
                <>
                  <h2 className="text-xl md:text-2xl font-bold">{currentTrack.title}</h2>
                  <p className="text-base md:text-lg text-muted-foreground">{currentTrack.artist}</p>
                </>
              ) : (
                <>
                  <h2 className="text-xl md:text-2xl font-bold">Track Collection</h2>
                  <p className="text-base md:text-lg text-muted-foreground">sofaspartan</p>
                </>
              )}
            </div>
          </div>
              ...
              {tracks.map((track) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 md:gap-4 p-2 md:p-4 hover:bg-white/10 transition-all duration-200 cursor-pointer group rounded-lg ${
                      isCurrentTrack ? 'bg-primary/10' : ''
                    }`}
                    onClick={() => onTrackSelect(track)}
                  >
                    <div className="text-muted-foreground text-sm md:text-base w-6 flex-shrink-0 group-hover:text-white transition-colors">
                      {track.id}
                    </div>
                    <div className="relative aspect-square w-10 h-10 md:w-16 md:h-16 rounded-lg overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform duration-200">
                      <img 
                        src={track.albumArt} 
                    ...
                    <div className="min-w-0 flex-grow">
                      <div className="flex items-center gap-2">
                        <h3 className="font-medium truncate text-sm md:text-base group-hover:text-white transition-colors">
                          {track.title}
                        </h3>
                        {isCurrentTrack && (
                    ...
                    <div className="flex-shrink-0">
                      <div className={`p-1 md:p-2 rounded-full h-7 w-7 md:h-10 md:w-10 flex items-center justify-center transition-all duration-200 ${
                        isCurrentTrack ? 'bg-primary group-hover:scale-110' : 'bg-primary/20 group-hover:bg-primary/30'
                      }`}>
                        {isCurrentTrack ? (
                          isPlaying ? (
                            <PauseIcon className="h-3 w-3 md:h-4 md:w-4 text-white" />
                          ) : (
                            <PlayIcon className="h-3 w-3 md:h-4 md:w-4 text-white" />
                          )
                        ) : (
                          <MusicalNoteIcon className="h-3 w-3 md:h-4 md:w-4 text-white/40 group-hover:text-white/60" />
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <div className="mt-6 md:mt-8 mb-16 md:mb-24 text-center">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
          <a 
            ...
            className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-white/5 hover:bg-white/10 transition-all duration-200 group"
          >
            <FontAwesomeIcon icon={faSoundcloud} className="w-4 h-4 md:w-5 md:h-5 text-white/60 group-hover:text-white transition-colors" />
            <span className="text-sm text-white/60 group-hover:text-white transition-colors">SoundCloud</span>
          </a>
          <a 
          ...
          >
            ...
            <span className="text-sm text-white/60 group-hover:text-white transition-colors">Instagram</span>
          </a>
          <a 
          ...
          >
            ...
            <span className="text-sm text-white/60 group-hover:text-white transition-colors">Email</span>
          </a>
        </div>
      </div>
    </div>
